...
import { Folder, Radio, Plus, CheckCircle2, Clock, Activity } from "lucide-react";
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    // 2. Összes feladat betöltése (Statisztikához)
    const unsubscribeTasks = onSnapshot(collection(db, "tasks"), (snapshot) => {
      setTasks(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Task)));
    });

    // 3. Összes időmérés betöltése (Statisztikához)
  ...
  // Mai napra szűrt munkaidő
  const startOfToday = new Date();
  startOfToday.setHours(0, 0, 0, 0);

  const todaysTotalSeconds = timeEntries
    .filter(entry => {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">
            {getGreeting()}, <span className="text-sona">{user?.email?.split('@')[0] || "Munkatárs"}</span>!
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl border border-neutral-800 bg-[#111111] p-5 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">Mai Munkaidő</h3>
          </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-4">
          <h2 className="text-lg font-semibold text-white">Your live projects</h2>
