...
import { Layout, List, CalendarDays, Plus, MoreHorizontal, CheckSquare, Calendar, Clock, Flag, ChevronLeft, ChevronRight, Circle } from 'lucide-react'
import { createTask, updateTaskOrders } from '../actions'

// ITT A MEGOLDÁS: Csak innen importáljuk a Task típust, és letöröltük a régit!
...
export function TaskManager({ initialTasks, workspaceId, projectId, currentUserId, hasEditOthersPerm, hasDeleteOthersPerm }: Props) {
    ...
    // -- ÚJ: NAPTÁR ÁLLAPOT --
    const [currentDate, setCurrentDate] = useState(new Date()) // Az épp nézett hónap

    // -- MODAL ÁLLAPOTOK --
    const [selectedTask, setSelectedTask] = useState<Task | null>(null)
    const [isModalOpen, setIsModalOpen] = useState(false)

    // Amikor a Modal visszaszól, hogy megváltozott a kártya (Optimista UI)
    const handleTaskUpdate = (updatedTask: Task) => {
        setTasks(tasks.map(t => t.id === updatedTask.id ? updatedTask : t))
        router.refresh() // Frissítjük a szervert is
    }

    // Amikor a Modal visszaszól, hogy törölt egy kártyát
    ...
    const handleDragStart = (e: React.DragEvent, taskId: string) => {
        ...
        // Hogy szép maradjon a kártya amíg húzod
    ...
    const handleDrop = async (e: React.DragEvent, targetStatus: string) => {
        ...
        // 1. Kiszámoljuk, hova dobták (Y koordináta alapján)
        const container = e.currentTarget.querySelector('.task-list-container')
        let targetIndex = -1 // -1 jelenti, hogy a lista legvégére kerül

        if (container) {
            const cards = Array.from(container.querySelectorAll('[data-task-id]'))
            for (let i = 0; i < cards.length; i++) {
                const rect = cards[i].getBoundingClientRect()
                // Ha az egér a kártya közepe felett van, akkor elé szúrjuk be
        ...
        // 2. Szétválogatjuk a feladatokat
        ...
        // 3. Beillesztjük a mozgatott kártyát az új helyére
        draggedTask.status = targetStatus
        if (targetIndex === -1) {
            targetColumnTasks.push(draggedTask) // A végére
        } else {
            targetColumnTasks.splice(targetIndex, 0, draggedTask) // A kiszámolt helyre
        }

        // 4. Újra-számozzuk az adott oszlop kártyáinak pozícióját (0, 1, 2, 3...)
        targetColumnTasks = targetColumnTasks.map((t, idx) => ({ ...t, position: idx }))

        // 5. Azonnali UI frissítés (Optimista frissítés)
        const newTasksState = [...otherTasks, ...targetColumnTasks]
        setTasks(newTasksState)

        // 6. Mentés az adatbázisba a háttérben
        ...
        if (result?.error) {
            ...
            router.refresh() // Visszaállítjuk, ha hiba volt
        }
    }
    ...
    const handleAddTask = async (e: React.FormEvent, status: string) => {
        ...
        if (result.success && result.task) {
            setTasks([...tasks, { ...result.task, position: 999 }]) // Új feladat a végére kerül
            setNewTaskTitle('')
            setAddingInColumn(null)
            router.refresh()
        }
        ...
        <div className="flex flex-col h-full flex-1 min-h-[500px]">
            ...
            {view === 'kanban' && (
                <div className="flex gap-6 overflow-x-auto pb-4 flex-1 items-start h-full">
                    {COLUMNS.map((column) => {
                        // SZŰRÉS ÉS RENDEZÉS A POZÍCIÓ ALAPJÁN
                            ...
                            >
                                ...
                                <div className="task-list-container flex flex-col gap-2 overflow-y-auto min-h-[30px]">
                                    {columnTasks.map(task => {
                                        // -- DÁTUM ÉS PRIORITÁS LOGIKA --

                                        // Prioritás színek beállítása
                                        ...
                                        // Határidő logikája (Piros, ha késésben, Sárga ha 3 napon belül van)
                                        ...
                                        if (task.due_date && task.status !== 'done') {
                                            ...
                                            if (diffDays < 0) dateColorClass = 'bg-red-500/10 text-red-500 font-semibold' // Lejárt!
                                            else if (diffDays <= 3) dateColorClass = 'bg-orange-500/10 text-orange-500 font-semibold' // Sürgető!
                                        }
                                        ...
                                        return (
                                            ...
                                            >
                                                {/* Prioritás Ikon a jobb felső sarokban */}
                                ...
                                {task.subtasks.filter(st => st.completed).length}/{task.subtasks.length}
                                                        ...
                                                        {(task.start_date || task.due_date) && (
                                                            <span className={`flex items-center gap-1 px-1.5 py-0.5 rounded ${dateColorClass}`}>
                                                                ...
                                                                {/* Ha mindkettő megvan: "Okt 10 - Okt 15" formátum */}
            ...
            {view === 'list' && (
                <div className="flex flex-col gap-6 overflow-y-auto pb-4 flex-1">
                    {COLUMNS.map((column) => {
                        ...
                        if (columnTasks.length === 0) return null // Ha üres az oszlop, listában nem mutatjuk

                        return (
                            <div key={column.id} className="bg-surface border border-border rounded-xl overflow-hidden shadow-sm">
                                {/* Lista Fejléc (Oszlop neve) */}
                                ...
                                <div className="divide-y divide-border">
                                    {columnTasks.map(task => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {/* Bal oldal: Státusz kör, Cím, Prioritás */}
...
{/* ================= NAPTÁR NÉZET ================= */}
      {view === 'calendar' && (() => {
        ...
        // Kikeressük a dátum nélküli (és még nem kész) feladatokat
        ...
        return (
          <div className="flex flex-col md:flex-row gap-6 h-full items-start flex-1">
            ...
            {/* DÁTUM NÉLKÜLI FELADATOK (Jobb oldalsáv) */}
