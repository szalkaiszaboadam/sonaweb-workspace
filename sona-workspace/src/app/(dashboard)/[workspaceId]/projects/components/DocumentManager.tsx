...
export type ProjectDocument = {
    ...
    content: string
    folder_name?: string
    position?: number // ÚJ: Sorrend a mappán belül
    updated_at: string
}
...
const handleContentChange = (html: string) => {
    ...
    return (
        <div className="flex h-[calc(100vh-12rem)] min-h-[600px] border border-border rounded-xl bg-surface shadow-sm overflow-hidden relative">
            ...
            <div
                className={`absolute lg:relative z-20 h-full bg-surface border-r border-border flex flex-col transition-all duration-300 ease-in-out ${isLeftPanelOpen ? 'w-72 translate-x-0' : 'w-72 -translate-x-full lg:translate-x-0 lg:w-0 lg:border-none opacity-0'
                    }`}
            >
                        ...
                        return (
                            ...
                            >
                                <div
                                    ...
                                    className={`group flex items-center justify-between px-2 py-1.5 rounded-md cursor-pointer transition-colors border ${activeDragTarget === `folder-${folder}` ? 'bg-primary/5 border-primary/50 border-dashed' : 'hover:bg-sona-neutral/10 border-transparent'}`}
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <button
                                            ...
                                            className="p-1 text-sona-neutral hover:text-primary"
                                            title="Új dokumentum"
                                        >
                                ...
                                {isExpanded && (
                                    <div className="flex flex-col ml-6 pl-2 border-l border-border/50 gap-0.5 mt-1 min-h-[10px]">
                                        ...
                                        {folderDocs.map(doc => (
                                            <div
                                                key={doc.id}
                                                // DOKUMENTUM DRAG & DROP
                                                draggable
                                                onDragStart={(e) => handleDragStart(e, 'doc', doc.id)}
                                                onDragEnd={handleDragEnd}
                                                onDragOver={(e) => handleDragOver(e, `doc-${doc.id}`)}
                                                onDrop={(e) => handleDrop(e, 'doc', doc.id)}
                                                onClick={() => { setActiveDocId(doc.id); if (window.innerWidth < 1024) setIsLeftPanelOpen(false); }}
                                                className={`group flex items-center justify-between px-2 py-1.5 rounded-md cursor-pointer transition-colors border text-sm ${activeDragTarget === `doc-${doc.id}` ? 'border-primary/50 border-dashed bg-primary/5' : activeDocId === doc.id ? 'bg-primary/10 text-primary font-medium border-transparent' : 'hover:bg-sona-neutral/10 text-sona-neutral hover:text-foreground border-transparent'}`}
                                            >
                        ...
                        {activeDoc ? (
                            editingTitleId === activeDoc.id ? (
                                <div className="flex items-center gap-2 flex-1">
                                    <input autoFocus type="text" value={editTitleValue} onChange={e => setEditTitleValue(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleUpdateTitle(activeDoc.id); if (e.key === 'Escape') setEditingTitleId(null) }} className="flex-1 text-lg font-bold bg-background border border-primary px-3 py-1 rounded-md focus:outline-none" />
                                ...
                                <div className="flex items-center gap-3 group min-w-0">
                                    ...
                                    <button onClick={() => { setEditingTitleId(activeDoc.id); setEditTitleValue(activeDoc.title) }} className="opacity-0 group-hover:opacity-100 p-1 text-sona-neutral hover:bg-sona-neutral/10 rounded-md transition-all shrink-0"><Edit3 className="w-4 h-4" /></button>
                                </div>
                            )
                ...
                <div className="flex-1 overflow-y-auto">
                    {activeDoc ? (
                        <div className="max-w-4xl mx-auto w-full p-6 sm:p-10 h-full">
                            <RichTextEditor key={activeDoc.id} content={activeDoc.content} onChange={handleContentChange} />
                        </div>
                    ) : (
